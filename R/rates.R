...
level_O2 <- function(raw, config, blanks)
{
  df <-
    ...
    dplyr::select(.data$measurement:.data$sensor, .data$fluor)
  ...
  df <-
    ...
    dplyr::mutate(O2 = config$O20 + config$f0 * (.data$ft - .data$fluor) / .data$fluor / .data$ft / config$Ksv) %>%
    dplyr::select(.data$measurement:.data$time, .data$O2)
}
...
calc_ocr <- function(levels, config)
{
  with(
    config,
    {
      ...
      gaps <- purrr::map_dfr(indices, ~fill_O2(levels, idx = .x))
      ...
      levels %>%
        ...
        dplyr::mutate(
          ...
          dO2_dt = pracma::savgol(.data$O2, 7, 2, 1) / pracma::savgol(.data$time, 7, 2, 1),
          dO2_dt = replace(.data$dO2_dt, 1:3, 0),
          d2O2_dt2 = pracma::savgol(.data$O2, 7, 2, 2) / pracma::savgol(.data$time, 7, 2, 1) ^ 2,
          d2O2_dt2 = replace(.data$d2O2_dt2, 1:3, 0),
          exp_tag = exp((Kaw + Kw) * .data$time),
          exponent = .data$exp_tag * (Kaw * O20 + Kw * (.data$sg_O2 + .data$dO2_dt / Kp)),
          integral = calc_integral(.data$time, .data$exponent),
          O2w = (.data$O2[[1]] + .data$integral) / .data$exp_tag,
          ocr_raw = Kac * O20 - (Kac + Kc) * .data$sg_O2 + Kc * .data$O2w - (Kac + Kp + Kc) / Kp * .data$dO2_dt - .data$d2O2_dt2 / Kp,
...
level_pH <- function(raw, config)
{
  ...
  raw %>%
    ...
    dplyr::select(.data$measurement:.data$time, .data$pH)
}
